...
import { Modal, Form, Button, Input, DatePicker, Select, InputNumber, Row, Col } from 'antd';
...
function ConsultedServiceForm({
    ...
    const handleServiceChange = (serviceId) => {
        ...
        if (selected) {
            ...
            setTimeout(() => {
                ...
                form.setFieldsValue({ finalPrice: (preferentialPrice || 0) * (quantity || 1) });
            }, 0);
        }
    };
    ...
    const handleToothSelection = (selectedTeeth) => {
        ...
        form.setFieldsValue({
            ...
            quantity: unit === 'Răng' ? (selectedTeeth.length || 1) : form.getFieldValue('quantity'),
        });
    ...
    return (
        <>
            <Modal
                open={visible}
                title={isViewMode ? "Xem Dịch vụ" : (initialValues ? "Sửa Dịch vụ" : "Thêm Dịch vụ")}
                width={1000}
                onCancel={onCancel}
                footer={[
                    <Button key="back" onClick={onCancel}>{isViewMode ? 'Đóng' : 'Hủy'}</Button>,
                    !isViewMode && <Button key="submit" type="primary" onClick={handleOk}>Lưu</Button>,
                ]}
            >
                <Form
                    form={form}
                ...
                >
                    <Row gutter={16}>
                        <Col xs={24} md={8}>
                            <Form.Item name="consultationDate" label="Ngày tư vấn" rules={[{ required: true }]}>
                        ...
                        <Col xs={12} md={6}>
                            <Form.Item name="quantity" label="Số lượng">
                                <InputNumber disabled={isViewMode} min={1} style={{ width: '100%' }} />
                            </Form.Item>
                        </Col>
                        <Col xs={24} md={8}>
                        ...
                        <Col xs={24} md={12}>
                            <Form.Item name="officialWarranty" label="Bảo hành chính hãng">
                                <Select
                                    disabled={isViewMode}
                                    options={OFFICIAL_WARRANTIES.map(w => ({ label: w, value: w }))}
                                    allowClear
                                />
                            </Form.Item>
                        </Col>
                        <Col xs={24} md={12}>
                            <Form.Item name="clinicWarranty" label="Bảo hành uy tín">
                                <Select
                                    disabled={isViewMode}
                                    options={CLINIC_WARRANTIES.map(w => ({ label: w, value: w }))}
                                    allowClear
                                />
                            </Form.Item>
                        </Col>
                        <Col xs={24} md={8}>
                            <Form.Item name="consultingDoctorId" label="BS Tư vấn">
                                <Select
                                    disabled={isViewMode}
                                    placeholder="Chọn BS"
                                    options={doctorsList.map(e => ({ label: e.fullName, value: e.id }))}
                                    allowClear
                                />
                            </Form.Item>
                        </Col>
                        <Col xs={24} md={8}>
                            <Form.Item name="consultingSaleId" label="Sale Tư vấn">
                                <Select
                                    ...
                                    options={salesList.map(e => ({ label: e.fullName, value: e.id }))}
                                    allowClear
                                />
                            </Form.Item>
                        </Col>
                        ...
                        <Form.Item name="denormalized" hidden><Input /></Form.Item>
                    </Row>
                </Form>
            </Modal>
            <ToothSelectionModal
                ...
                initialSelected={
                    (form.getFieldValue('toothPositions') || '')
